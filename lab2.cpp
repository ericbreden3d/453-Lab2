    ...
    for (int i = 0; i < dims[0]; i++) {
        for (int j = 0; j < dims[1]; j++) {
            if (this_coord[0] == i && this_coord[1] == j) {
                ...
                MPI_Isend(A.get_1d(), sub_n * sub_n, MPI_INT, targ_A, 0, cart_comm, &req);
                MPI_Isend(B.get_1d(), sub_n * sub_n, MPI_INT, targ_B, 0, <<cart_comm, &req);
                int A_coord[2];
                MPI_Cart_rank(cart_comm, targ_A, 2, A_coord);
